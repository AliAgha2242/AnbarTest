...
#Region " Windows Form Designer generated code "
    ...
    Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmabTafkik_New))
...
#Region "Upgrade Support_For2Instance"
    ...
    Private Shared m_vb6formdefinstance As FrmabTafkik_New
    Private Shared m_initializingdefinstance As Boolean

    Public Shared Property DefInstance() As FrmabTafkik_New

        Get

            If m_vb6formdefinstance Is Nothing OrElse m_vb6formdefinstance.IsDisposed Then
                m_initializingdefinstance = True
                m_vb6formdefinstance = New FrmabTafkik_New
        ...
        Set(ByVal Value As FrmabTafkik_New)
...
#End Region
    ...
    Private Sub FrmabTafkik_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        DvTafkik = _rule.GetTafkikByAnbar(cn, gAnbarSN).Tables(0).DefaultView
    ...
    Private Sub GridShobe_SelectionChanged(sender As System.Object, e As System.EventArgs) Handles GridShobe.SelectionChanged
        If GridShobe.RowCount > 0 AndAlso GridShobe.CurrentRow.Cells("ShobeDS").Text <> "" Then
            ...
            DarkhastHa = DarkhastHa.ToTable(True, {"DarkhastSN", "DarkhastNo", "DarkhastDate", "DarkhastKonandeh", "DarkhastDS"}).DefaultView()
    ...
    Private Sub BtnErsalForHavaleh_Click(sender As System.Object, e As System.EventArgs) Handles BtnErsalHavaleh.Click
        ...
        Dim DtKalaForSend As DataTable = New DataView(DvKalaForSend, "ShobeSN=" & GridShobe.CurrentRow.Cells("ShobeSN").Value, "", DataViewRowState.CurrentRows).ToTable.Copy

        If DtKalaForSend.Rows.Count = 0 Then
            NetSql.Common.CSystem.MsgBox("موردی برای ارسال به  " + GridShobe.CurrentRow.Cells("ShobeDS").Text + " وجود ندارد", MsgBoxStyle.Exclamation, "")
            Exit Sub
        End If
        ...
        Try
            If _
            NetSql.Common.CSystem.MsgBox((DtKalaForSend.Rows.Count).ToString + " مورد کالا برای ارسال به " + GridShobe.CurrentRow.Cells("ShobeDS").Text + " انتخاب شده است." + vbCr +
                                         "آیا از ادامه عملیات ارسال اطمینان دارید؟",
                       MsgBoxStyle.YesNo Or MsgBoxStyle.DefaultButton2 Or MsgBoxStyle.Exclamation Or MsgBoxStyle.Question Or
                ...
                Dim DarkhastNoTbl As DataTable = New DataView(DtKalaForSend, "", "", DataViewRowState.CurrentRows).ToTable(True, {"DarkhastNo", "DarkhastDS", "Tozih"})
                If DarkhastNoTbl.Rows.Count = 1 AndAlso IsNumeric(DarkhastNoTbl.DefaultView(0)("DarkhastNo").ToString) Then
                    TozihTitrSanad += "  بابت درخواست  " + DarkhastNoTbl.DefaultView(0)("DarkhastNo").ToString + If(DarkhastNoTbl.DefaultView(0)("DarkhastDS").ToString <> "", " (" + DarkhastNoTbl.DefaultView(0)("DarkhastDS").ToString + ")", "")
                Else
                    ...
                    For i As Integer = 0 To Strdarkhast1.Length - 1
                        Dim CntDarkhast As Integer = DtKalaForSend.Compute("Count(DarkhastNo)", "DarkhastNo='" & Strdarkhast1(i).ToString & "'")
                        DarkhastNoTbl.DefaultView.RowFilter = "0=0"
                        DarkhastNoTbl.DefaultView.RowFilter = "DarkhastNo='" & Strdarkhast1(i).ToString & "'"
                        Dim DarkhastDS As String = ""
                        If DarkhastNoTbl.DefaultView.Count > 0 Then
                            DarkhastDS = DarkhastNoTbl.DefaultView(0)("DarkhastDS").ToString
                        End If
                        TozihTitrSanad += "<" + Strdarkhast1(i) + If(DarkhastDS <> "", " (" & DarkhastDS & ")", "") + ": " + CntDarkhast.ToString + "م>"
                ...
                Dim ArzeshHavaleh As Long = DtKalaForSend.Compute("Sum(Rial)", "")

                TozihTitrSanad = "ارزش ریالی حواله : " + ArzeshHavaleh.ToString + "  ریال - " + "(م : مورد از اقلام درخواست) - " + TozihTitrSanad

                Dim ListTafkikSnStr As String = ""
                Dim ListTafkikSNs As DataTable = New DataView(DtKalaForSend, "", "", DataViewRowState.CurrentRows).ToTable(True, "ListTafkikSN")
                ...
                Dim ListTafkikHaSNTbl As DataTable = New DataView(DtKalaForSend, "", "", DataViewRowState.CurrentRows).ToTable(True, {"ListTafkikSN", "ListTafkikHaSN"})
                Dim l As DataView = New DataView(ListTafkikHaSNTbl, "", "ListTafkikHaSN", DataViewRowState.CurrentRows)

                Dim DarkhastSnTbl As DataTable = New DataView(DtKalaForSend, "", "", DataViewRowState.CurrentRows).ToTable(True, "DarkhastSn")
                Dim DarkhastSn As String = ""
        ...
        Finally
            'DvKalaForSend.DefaultView.RowFilter = "0=0"
            'GridKalaForSend.RemoveFilters()
        End Try
